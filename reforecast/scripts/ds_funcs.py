...
def get_gfs_local(_inittime):
    ...
    datapath = datadir + init_date + '/models/gfsds/*%s*'%_inittime
    flist = [f for f in glob(datapath) if (('.grib2' in f)&('idx' not in f))]
    flist = np.array(flist)[np.argsort(flist)]

    isobaric = xr.open_mfdataset(flist, engine='cfgrib', concat_dim='valid_time', backend_kwargs={'filter_by_keys':{'typeOfLevel': 'isobaricInhPa'}})
    isobaric = isobaric.rename({'isobaricInhPa':'level', 'latitude':'lat', 'longitude':'lon'})
    # print(isobaric)
    
    surface = xr.open_mfdataset(flist, engine='cfgrib', concat_dim='valid_time', backend_kwargs={'filter_by_keys':{'typeOfLevel': 'surface', 'stepType':'instant'}})
    surface = surface.rename({'latitude':'lat', 'longitude':'lon'})
    # print(surface)
    ...
    msgs = np.array([[msg for msg in pygrib.open(f).read() if 'Precipitation' in str(msg)] for f in flist])
    msgno = [None]
    for i, msg in enumerate(msgs):
        if len(msg) >= 1:
            msgno.append(int(str(msg[-1]).split(':')[0]))

    qpf = [np.zeros(surface.orog.isel(time=0).shape)]
    for i in range(len(flist[1:])): 
        i += 1
        # print(i, flist[i])

        grbs = pygrib.open(flist[i])
        grb = grbs.message(msgno[i])
        print('Used %s'%grb)

        if i>1:
            priorgrbs = pygrib.open(flist[i-1])
            priorgrb = priorgrbs.message(msgno[i-1])
            qpf.append(grb.values - priorgrb.values)
        else:
            qpf.append(grb.values)

    qpf = np.array(qpf)
    qpf = xr.DataArray(qpf).rename({'dim_0':'time', 'dim_1':'lat', 'dim_2':'lon'}).transpose('time', 'lat', 'lon')
    surface['qpf'] = qpf
...
def get_nam_local(_inittime):
    ...
    datapath = datadir + init_date + '/models/namds/*%s_00*'%_inittime[:-2]
    flist = [f for f in glob(datapath) if 'idx' not in f]
    flist = np.array(flist)[np.argsort(flist)]
    ...
    isobaric = isobaric.rename({'valid_time':'time'})#.load()
    surface = surface.rename({'valid_time':'time'}).drop('surface')#.load()
    ...
    for i in range(len(flist[1:])): 
        ...
        # Using 3-h acc, just take them as-is
        qpf.append(grb.values)

    qpf = np.array(qpf)
    qpf = xr.DataArray(qpf).rename({'dim_0':'time', 'dim_1':'y', 'dim_2':'x'}).transpose('time', 'y', 'x')
    surface['qpf'] = qpf    
